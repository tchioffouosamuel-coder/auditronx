...
function KpiCard({ label, value, tone = "neutral" }) {
  ...
  return (
    <div className="rounded-xl border border-ink-100 bg-white p-4 shadow-sm transition hover:shadow-md">
      ...
      <div className={`mt-1.5 text-3xl font-bold tabular-nums ${tones[tone]}`}>
        {value}
      </div>
    </div>
  );
...
function PeopleList({ title, people = [], tone = "neutral" }) {
  ...
  return (
    <section className="rounded-xl border border-ink-100 bg-white p-4 shadow-sm sm:p-6">
      <h2 className={`mb-4 text-sm font-medium ${colors[tone]}`}>
        {title} ({people.length})
      </h2>
      {people.length === 0 ? (
            ...
            <tbody className="divide-y divide-ink-100">
              {people.map((person) => (
                <tr key={person.enseignant_id}>
                  <td className="whitespace-nowrap px-2 py-3 font-medium text-ink-800">
                    {person.nom || "—"}
                  </td>
      ...
      {!data && (
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
          {[0, 1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-20 animate-pulse rounded-xl border border-ink-100 bg-ink-100/50"
            />
          ))}
        </div>
      )}
