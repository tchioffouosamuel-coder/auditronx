...
class RetardsController extends Controller
{
    ...
    private function donneesFiche(Enseignant $enseignant, Carbon $debut, Carbon $fin, RetardCalculator $retards): array
    {
        ...
        for ($date = $debut->copy(); $date->lte($fin); $date->addDay()) {
            ...
            if (!$futur) {
                $totalRetard += $retard; $totalAnticipation += $anticipation;
                if ($absent) $totalPeriodesAbsence += $periodesPrevues; else $totalPeriodesPresence += $periodesRattraper;
            }
            $details[] = [
                'date' => $date->format('d/m/Y'), 'jour' => $this->nomJour($date->isoWeekday()), 'nb_cours' => $coursDuJour->count(),
                'heure_debut_prevue' => $premierCours->heure_debut, 'heure_fin_prevue' => $dernierCours->heure_fin,
                'heure_arrivee' => $presence?->heure_arrivee?->format('H:i'), 'heure_depart' => $presence?->heure_depart?->format('H:i'),
                'retard_minutes' => $retard, 'anticipation_minutes' => $anticipation, 'periodes_a_rattraper' => $periodesRattraper,
                'futur' => $futur, 'absent' => $absent, 'est_signale' => $signale, 'type_signalement' => $signalement?->motif,
            ];
        }

        return [
            'enseignant' => $enseignant, 'mois' => $debut->format('m'), 'annee' => $debut->year, 'emploi_par_jour' => $emploiParJour,
            'details' => $details, 'total_retard_minutes' => $totalRetard, 'total_anticipation_minutes' => $totalAnticipation,
            'total_periodes_a_rattraper' => $totalPeriodesPresence, 'total_periodes_absence' => $totalPeriodesAbsence,
            'jours_attendus' => $joursAttendus, 'presences_valides' => $presencesValides,
    ...
    private function ligneAssiduite(Enseignant $enseignant, Carbon $debut, Carbon $fin, array $ligne): array
    {
        ...
        $presences = $enseignant->presences()
            ...
            ->whereNotNull('heure_arrivee')->pluck('date')
            ->map(fn ($date) => Carbon::parse($date)->toDateString())->unique();
        $signalements = $enseignant->signalements()
            ->whereBetween('date', [$debut->toDateString(), $fin->toDateString()])
            ->pluck('date')->map(fn ($date) => Carbon::parse($date)->toDateString())->all();
        $attendues = $datesAttendues->unique()->count();
        $enregistrees = $presences->intersect($datesAttendues->unique())->count();

        return [
            'nom' => $enseignant->nom, 'tel' => $enseignant->tel, 'fonction' => $enseignant->fonction,
            'matricule' => $enseignant->matricule, 'specialite' => $enseignant->section,
            'nb_cours' => $emplois->count(), 'total_periodes' => $totalPeriodes,
            'nbre_attendues' => $attendues, 'nbre_enregistrees' => $enregistrees,
            'taux' => $attendues > 0 ? round($enregistrees / $attendues * 100, 2) : 0,
            'dates_signalement' => $signalements, 'jours_retard' => $ligne['jours_retard'] ?? 0,
