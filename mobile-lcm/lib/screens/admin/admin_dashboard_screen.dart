...
class _DashboardPeopleSection extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Card(child: Padding(padding: const EdgeInsets.fromLTRB(12, 12, 12, 4), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (people.isEmpty) const Padding(padding: EdgeInsets.symmetric(vertical: 16), child: Text('Aucune personne.', style: TextStyle(color: AuditronColors.ink500)))
      else ...people.map((person) {
        final cours = (person['cours'] as List?)?.whereType<Map>().map((c) => '${c['classe'] ?? '—'} ${c['heure_debut'] ?? ''}-${c['heure_fin'] ?? ''}').join(' · ') ?? '';
        final horaires = [if (person['heure_arrivee'] != null) 'Arrivée ${person['heure_arrivee']}', if (person['heure_depart'] != null) 'Départ ${person['heure_depart']}', if (person['minutes_retard'] != null && person['minutes_retard'] != 0) '${person['minutes_retard']} min de retard'].join(' · ');
        return ListTile(dense: true, contentPadding: EdgeInsets.zero, leading: Icon(icon, color: color, size: 18), title: Text(person['nom']?.toString() ?? '—'), subtitle: Text([person['matricule'], horaires, cours].where((v) => v != null && v.toString().isNotEmpty).join(' · '), overflow: TextOverflow.ellipsis, maxLines: 2));
      }),
    ])));
  }
}
