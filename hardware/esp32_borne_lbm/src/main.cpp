...
static void syncWithApi()
{
    ...
    // `body` (jusqu'à SYNC_BODY_JSON_CAPACITY = 160 Ko avec les photos) est
    // construit puis sérialisé dans ce bloc, qui se referme AVANT d'ouvrir la
    // connexion TLS : le garder vivant pendant la poignée de main mbedTLS
    // (elle-même gourmande) cumule les deux plus gros consommateurs de tas au
    // même instant — cause du "SSL - Memory allocation failed" observé sur
    // esp32dev_borne/ (module sans PSRAM, plus tendu) ; corrigé ici aussi par
    // prudence.
    String payload;
    {
...
void setup()
{
    ...
    // besoin du rôle AP (voir setupBle() ci-dessous). Mode réglé ici (pas
    // encore de trafic radio significatif) mais WiFi.begin() est décalé après
    // l'init BLE (voir plus bas) : les deux radios démarrant leurs pics de
    // courant en même temps que la caméra pouvaient faire chuter la tension
    // sous le seuil de brownout au boot (rst:0xf BROWNOUT_RST en boucle).
    WiFi.mode(WIFI_STA);

    setupBle();

    // Laisse la radio BLE terminer son démarrage (advertising) avant de
    // démarrer la radio WiFi, pour étaler les pics de courant plutôt que de
    // les cumuler.
    delay(1500);
    Serial.println("[wifi] tentative de connexion au modem...");
    WiFi.begin(STA_SSID, STA_PASSWORD);
